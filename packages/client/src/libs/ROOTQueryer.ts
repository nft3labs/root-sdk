...
export interface QueryResponse {
  ...
  nft3Stats: ROOTStats
  nft3ReferrerStats: ROOTReferrerStats
}
...
export interface ROOTReferrerStats {
  invitees: number
...
export default class ROOTQueryer {
  ...
  private buildQuery(options: QueryOptions) {
    ...
    let content = `query ROOTQuery(${queryStr}) {
      ${bodys.join('\n')}
    }`
    if (querys.length === 0) {
